...
public class ReplacerItemWriter<T>  implements ResourceAwareItemWriterItemStream<T>,
InitializingBean {
	...
	public void write(List<? extends T> items) throws Exception {
		...
		setResource(((Resource) stepExecution.getExecutionContext().get("current.resource")));
		
		
		if(resource != null)
		 getOutputState().initializeBufferedWriter();
		
		System.out.println("Writing to flat file with " + items + " items.");
	...
	private class OutputState {
		...
		private void initializeBufferedWriter() throws IOException {
			...
			//File file = ((Resource) stepExecution.getExecutionContext().get("current.resource")).getFile();
			System.out.println("FFFFFFFF-->" + file);
